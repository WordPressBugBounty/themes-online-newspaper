...
<article class="filter-item <?php if(!has_post_thumbnail()) { echo esc_attr('no-feat-img');} ?>">
    <div class="blaze_box_wrap online-newspaper-card">
        <figure class="post-thumb-wrap">
            ...
            <?php if( $args[ 'categoryOption' ] && $args[ 'featuredPosts' ] ) online_newspaper_get_post_categories( get_the_ID(), 2 ); ?>
        </figure>
        <div class="post-element">
            <h2 class="post-title"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
            <div class="post-meta">
                <?php if( $args[ 'authorOption' ] ) online_newspaper_posted_by(); ?>
                <?php if( $args[ 'dateOption' ] ) online_newspaper_posted_on(); ?>
                <?php
            ...
            <?php
                ...
                do_action( 'online_newspaper_section_block_view_all_hook', array(
                    'buttonOption'    => isset( $args[ 'buttonOption' ] ) ? $args[ 'buttonOption' ] : false
                ));
            ?>
        </div>
    </div>
