...
jQuery(document).ready(function($) {
    ...
    function onlineNewspaperClickOutSideElm(elm, callback) {
    ...
    $( ".online-newspaper-repeater-control" ).each(function() {
        var container = $(this)
        ...
        container.on( "click", ".image-field .add-image-trigger", function(event) {
            ...
            frame.on('select', function() {
                ...
                _this.parents( ".online-newspaper-repeater-item" ).addClass("popupActive")
        ...
        container.on( "click", ".fontawesome-icon-picker .icons-list i", function() {
            ...
            if( ! _this.hasClass('selected') ) {
                ...
                _this.parents( ".online-newspaper-repeater-item" ).find( ".item-heading-wrap .item-heading" ).text(newValue.split("-")[1])
            }
            repeater_value_refresh(_this);
        })

        // sortable handler
        container.find( ".online-newspaper-repeater-control-inner" ).sortable({
            orientation: "vertical",
            items: "> .online-newspaper-repeater-item",
            update: function (event, ui) {
                $(this).find( " > .online-newspaper-repeater-item .remove-item" ).show()
                $(this).find( ".online-newspaper-repeater-item:first .remove-item" ).hide()
        ...
        container.on( "click", ".display-icon", function() {
            var _this = $(this);
            _this.parents('.online-newspaper-repeater-item').find("input[data-key='item_option']").val(function(index,current) {
                if( current === 'show' ) {
                    return 'hide';
                } else {
                    return 'show';
            ...
            _this.parents( ".online-newspaper-repeater-item" ).toggleClass( "not-visible visible" )
            repeater_value_refresh(_this)
        })
        ...
        container.on( "click", ".remove-item", function(e) {
            e.preventDefault();
            var controller = $(this).parents( ".online-newspaper-repeater-control" ), toRemove = $(this).parents( ".online-newspaper-repeater-item" );
        ...
        container.on( "click", ".add-new-item", function(e) {
            ...
            onlineNewspaperClickOutSideElm( fieldsPopup, function() {
                fieldsPopup.parents( ".online-newspaper-repeater-item" ).removeClass("popupActive")
        ...
        container.on( "click", ".online-newspaper-repeater-item.visible .settings-icon", function() {
            $(this).parents(".online-newspaper-repeater-item").siblings().removeClass("popupActive").find(".item-control-fields").removeClass("isShow").addClass("isHidden")
            $(this).parents(".online-newspaper-repeater-item").siblings().find( ".fontawesome-icon-picker .icons-list" ).hide()
            $(this).parents(".online-newspaper-repeater-item").toggleClass("popupActive")
            ...
            // close popup on outside click
            var fieldsPopup =  $(this).parents(".online-newspaper-repeater-item").find( ".item-control-fields.isShow" )
            onlineNewspaperClickOutSideElm( fieldsPopup, function() {
                fieldsPopup.parents( ".online-newspaper-repeater-item" ).removeClass("popupActive")
                fieldsPopup.removeClass( "isShow" ).addClass( "isHidden" )
            })
        ...
        container.on( "change keyup", ".online-newspaper-repeater-item .repeater-field-value-holder", function() {
            var _this = $(this)
            repeater_value_refresh(_this)
        })
    ...
    function repeater_value_refresh( _this ) {
        var controlValue = [], container =  ( _this.hasClass( "online-newspaper-repeater-control" ) ) ? _this : _this.parents( ".online-newspaper-repeater-control" );
        container.find( ".online-newspaper-repeater-item" ).each(function() {
            var newValue = {}
