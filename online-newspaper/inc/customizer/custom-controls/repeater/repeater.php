 ...
 * @since 1.0.0
 */
class Online_Newspaper_WP_Custom_Repeater extends \Online_Newspaper_WP_Base_Control {
    /**
     * Control Type
     */
     ...
     * Main function
     * 
     * Online_Newspaper_WP_Custom_Repeater class name 
     */
    ...
    function enqueue() {
        wp_enqueue_media();
        wp_enqueue_style( 'fontawesome', get_template_directory_uri() . '/assets/lib/fontawesome/css/all.min.css', [], '6.4.2', 'all' );
        wp_enqueue_style( 'online-newspaper-wp-repeater', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.css', [], ONLINE_NEWSPAPER_VERSION, 'all' );
        wp_enqueue_script( 'online-newspaper-wp-repeater', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.js', [ 'jquery' ], ONLINE_NEWSPAPER_VERSION, [ 'strategy' => 'defer', 'in_footer' => true ] );
    }
    ...
    public function render_content() {
        ...
        $item_count = 1;
        ?>
        <div class="online-newspaper-repeater-control">
            <label class="customize-control-title"><?php echo esc_html( $this->label ); ?></label>
            <?php if( $this->description ) { ?>
                <span class="description customize-control-description"><?php echo esc_html( $this->description ); ?></span>
            <?php } ?>
            <div class="online-newspaper-repeater-control-inner">
                <?php 
                $row_label = $this->row_label;
                 foreach( $control_values as $control_value_key => $control_value ) :
                  ...
                  ?>
                    <div class="online-newspaper-repeater-item <?php if( $control_value->item_option === 'show' ) : echo 'visible'; else : echo 'not-visible'; endif; ?>">
    ...
    function render_control( $field_key, $field, $current_item_value ) {
        $type = $field->type;
        $label = isset( $field->label ) ? $field->label : esc_html__( 'Item Label', 'online-newspaper' );
        ...
        switch( $type ) {
            ...
            case 'image' : ?>
                <div class="single-control image-field">
                    ...
                    <div class="image-holder">
                        ...
                        <div class="add-image-trigger <?php if( $current_item_value->$field_key ) echo 'no-trigger'; ?>"><?php echo esc_html__( 'Upload Image', 'online-newspaper'); ?></div>
            ...
            <?php
                break;
            case 'fontawesome-icon-picker' : $icons_list = online_newspaper_get_all_fontawesome_icons( $families ); ?>
                <div class="single-control fontawesome-icon-picker">
                    ...
                    <div class="icon-holder">
                        ...
                        <div class="icons-list" style="display:none;">
                            <input class="icon-search-input" type="text" placeholder="<?php echo esc_attr__( "Type to search", 'online-newspaper' ); ?>"/>
                            <?php
            ...
            case 'checkbox': ?>
                ...
                break;
            case 'number': ?>
                    <div class="single-control number-field">
                        <h2 class="control--item-label"><?php echo esc_html( $label );?></h2>
                        <?php if( $description ) echo '<span class="control--item-description">', esc_html( $description ) ,'</span>'; ?>
                        <div class="control-inner">
                            <input class="repeater-field-value-holder" data-default="<?php echo esc_attr( $default_value ); ?>" data-key="<?php echo esc_attr( $field_key ); ?>" type="number" value="<?php echo esc_attr( $current_item_value->$field_key ); ?>"/>
                        </div>
                    </div>
                <?php
                break;
            ...
            case 'alignment': ?>
                ...
                break;
            case 'social-share': 
                $icons_list = online_newspaper_get_all_social_share();
                ?>
                    <div class="single-control fontawesome-icon-picker">
                        <h2 class="control--item-label"><?php echo esc_html( $label ); ?></h2>
                        <?php if( $description ) { ?>
